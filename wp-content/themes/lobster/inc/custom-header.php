...
function lobster_custom_header_setup() {
	...
	register_default_headers( array(
		'Pattern One' => array(
			...
			'description'   => _x( 'Pattern One', 'header image description', 'cdc_lobster' )
		),
		'Pattern Two' => array(
			'url'           => '%s/images/headers/two_bg.jpg',
			'thumbnail_url' => '%s/images/headers/two_thumb.png',
			'description'   => _x( 'Pattern Two', 'header image description', 'cdc_lobster' )
		),
		'Pattern Three' => array(
			'url'           => '%s/images/headers/three_bg.jpg',
			'thumbnail_url' => '%s/images/headers/three_thumb.png',
			'description'   => _x( 'Pattern Three', 'header image description', 'cdc_lobster' )
		),
	) );
}
...
function lobster_admin_header_image() {
	...
	if ( !empty($cyber_dc_boot_theme_options['custom_header_display']) ) {
				...
				<div class="displaying-header-text home-jumbotron jumbotron col-xs-12" id="headimg" style="background: url(<?php header_image(); ?>) no-repeat scroll top; background-size: 1600px auto;">
					<h3 class="lobster-ch-intro" style="color:#<?php header_textcolor(); ?>;"><?php echo $cyber_dc_boot_theme_options['cust_header_headline_title']; ?></h3>
					<i style="color:#<?php header_textcolor(); ?>;" class="jumbo-icon icon-star-empty"></i>
					<p class="lead"><?php if ( is_front_page() || is_home() ) { echo $cyber_dc_boot_theme_options['cust_header_headline_text'];} elseif ( is_page() ){ echo '<small>'.bloginfo( 'description' ).'</small>';} ?></p>
				</div>
