...
add_action( 'wp_head', 'cyber_dc_boot_styles' );
 ...
 * @since 1.0
 */
function cyber_dc_boot_styles() {
	$cyber_dc_boot_theme_options = cyber_dc_boot_theme_options();
	?>
<style>
.container, .boxed #page, #main > .row, .home #primary article > .row, .archive #primary article > .row, .search #primary article > .row { max-width: <?php echo $cyber_dc_boot_theme_options['width']; ?>px; }
</style>
	<?php
}


add_action( 'wp_enqueue_scripts', 'cyber_dc_boot_add_js' );
if ( ! function_exists( 'cyber_dc_boot_add_js' ) ) :
/**
